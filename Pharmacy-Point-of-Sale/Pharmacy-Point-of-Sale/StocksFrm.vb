Public Class StocksFrm
    ...
    Public Sub DisplayProducts(ByVal qty As Integer, ByVal curStockQty As Integer, ByVal lotNumber As String, ByVal brandName As String, ByVal genericName As String, ByVal category As String, ByVal classification As String, ByVal unitCost As Decimal, ByVal currentStocksQuantity As Integer, ByVal expirationDate As String)
        ProductsDGV.ForeColor = Color.Black

        Dim medItems As New DataTable("PurchaseProductList")

        Try
            ProductsDGV.Rows.Add(qty, curStockQty, lotNumber, brandName, genericName, category, classification, unitCost, currentStocksQuantity, expirationDate)
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Display Products", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub

    Private Sub ProductsDGV_CellFormatting(sender As System.Object, e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles ProductsDGV.CellFormatting
        For i As Integer = 0 To ProductsDGV.Rows.Count - 1
            If (ProductsDGV.Rows(i).Cells(0).Value / 4) >= ProductsDGV.Rows(i).Cells(1).Value Then

                ProductsDGV.Rows(i).Cells(2).Style.ForeColor = Color.Red
                ProductsDGV.Rows(i).Cells(3).Style.ForeColor = Color.Red
                ProductsDGV.Rows(i).Cells(4).Style.ForeColor = Color.Red
                ProductsDGV.Rows(i).Cells(5).Style.ForeColor = Color.Red
                ProductsDGV.Rows(i).Cells(6).Style.ForeColor = Color.Red
                ProductsDGV.Rows(i).Cells(7).Style.ForeColor = Color.Red
                ProductsDGV.Rows(i).Cells(8).Style.ForeColor = Color.Red
                ProductsDGV.Rows(i).Cells(9).Style.ForeColor = Color.Red
    ...
    Private Sub SearchClassification()
        ...
        Try
            ...
            If Me.ClassificationTxt.Text <> "" Then
                className = Me.ClassificationTxt.Text
                Me.ClassificationDGV.Rows.Clear()
    ...
    Private Sub ClassificationTxt_TextChanged(sender As System.Object, e As System.EventArgs) Handles ClassificationTxt.TextChanged
