...
Public Class PharmacyStocksController : Inherits SqlControl
    ...
    Public Sub GetAllProductsInProdCategory(ByVal prodCatID As Integer, ByRef displayProducts As DisplayProducts)
        ...
        Try
            Dim getThisProductQuery As String = "SELECT MedicinesStockBatch.lotNumber, Medicines.brandName, Medicines.genericName, MedicineCategory.category,MedicineClassification.classification, Medicines.unitCost,PharmacyStocks.currentStocksQuantity, MedicinesStockBatchItems.expirationDate " & _
            ...
            Try
                While (reader.Read())
                    'displayProducts(reader("id"), reader("productCode"), reader("brandName"), reader("genericName"), reader("prodQuantity"), reader("prodPrice"))
    ...
    Public Sub GetAllProducts(ByRef displayProducts As DisplayProducts, ByVal batchID As Integer)
        ...
        Try
            'Dim getAllProducts As String = "SELECT Products.*, Pharmacy_Product_Stocks.prodQuantity FROM Products,Pharmacy_Product_Stocks WHERE Pharmacy_Product_Stocks.prodId=Products.id"
            Dim getAllProducts As String = "SELECT MedicinesStockBatch.lotNumber, Medicines.brandName, Medicines.genericName, MedicineCategory.category,MedicineClassification.classification, Medicines.unitCost,PharmacyStocks.currentStocksQuantity, MedicinesStockBatchItems.expirationDate " & _
                                            "FROM Medicines,PharmacyStocks,MedicinesStockBatch,MedicinesStockBatchItems, MedicineCategory, MedicineClassification " & _
    ...
    Public Sub SearchClassification(ByVal className As String, ByRef displayMedicineClassification As DisplayMedicineClassification)
        Try
    ...
    Public Sub SearchProduct(ByVal prodBrandGernericName As String, ByVal batchId As Integer, ByRef displayProducts As DisplayProducts)
        Try
            Dim searchProductQuery As String = "SELECT MedicinesStockBatch.lotNumber, Medicines.brandName, Medicines.genericName, MedicineCategory.category,MedicineClassification.classification, Medicines.unitCost,PharmacyStocks.currentStocksQuantity, MedicinesStockBatchItems.expirationDate " & _
                                            "FROM Medicines,PharmacyStocks,MedicinesStockBatch,MedicinesStockBatchItems, MedicineCategory, MedicineClassification " & _
    ...
    Public Sub GetAllProductsBelongsToClassification(ByVal classificationId As Integer, ByRef displayProducts As DisplayProducts)
        Try
            Dim getThisProductQuery As String = "SELECT MedicinesStockBatch.lotNumber, Medicines.brandName, Medicines.genericName, MedicineCategory.category,MedicineClassification.classification, Medicines.unitCost,PharmacyStocks.currentStocksQuantity, MedicinesStockBatchItems.expirationDate " & _
                                            "FROM Medicines,PharmacyStocks,MedicinesStockBatch,MedicinesStockBatchItems, MedicineCategory, MedicineClassification " & _
                                            "WHERE PharmacyStocks.batchID=MedicinesStockBatch.id AND PharmacyStocks.medicineID=Medicines.id AND PharmacyStocks.batchId=MedicinesStockBatchItems.batchId AND Medicines.id=MedicinesStockBatchItems.medicineId AND Medicines.categoryId=MedicineCategory.id AND Medicines.medClassificationId=MedicineClassification.id AND Medicines.medClassificationId=" & classificationId
            Dim sqlCommand As New SqlCommand(getThisProductQuery, Me.Connection)
            ...
            Try
                ...
                End While
            Catch ex As Exception
                MessageBox.Show(ex.Message, "Error: 0x101 - Get All Medicines belongs to classification (2)", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Finally
                ' Clossing connections
                reader.Close()
                sqlCommand.Dispose()
            ...
            End Try
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error: 0x101 - Get All Medicines belongs to classification (1)", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Public Sub FilterAllProducts(ByVal classId As Integer, ByVal catId As Integer, ByVal batchId As Integer, ByRef displayProducts As DisplayProducts)
        Try
            Dim getThisProductQuery As String = "SELECT MedicinesStockBatch.lotNumber, Medicines.brandName, Medicines.genericName, MedicineCategory.category,MedicineClassification.classification, Medicines.unitCost,PharmacyStocks.currentStocksQuantity, MedicinesStockBatchItems.expirationDate " & _
                                            "FROM Medicines,PharmacyStocks,MedicinesStockBatch,MedicinesStockBatchItems, MedicineCategory, MedicineClassification " & _
                                            "WHERE PharmacyStocks.batchID=MedicinesStockBatch.id AND PharmacyStocks.medicineID=Medicines.id AND PharmacyStocks.batchId=MedicinesStockBatchItems.batchId AND Medicines.id=MedicinesStockBatchItems.medicineId AND Medicines.categoryId=MedicineCategory.id" & _
